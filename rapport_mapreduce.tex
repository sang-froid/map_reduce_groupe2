...
à savoir $C_0=[35,80,0.5,120]$ (profil rural) et $C_1=[22,45,2.5,500]$ (profil urbain).
...
Ces tests ont été réalisés sur données synthétiques avec un timeout de 20\,s. Le temps \emph{total} inclut le coût réseau IoT (1\,Mbps pour le classique, LAN\,Gigabit pour 15 sommes partielles MapReduce).
...
(32\,Go à 1\,Mbps) avant même de calculer, tandis que MapReduce livre le résultat en $\approx$8,7\,h
($9\times$ de gain).
...
\begin{enumerate}
\item L'implémentation utilise \texttt{multiprocessing.Pool} sur une seule machine ; une vraie distribution sur réseau (Spark, Hadoop) améliorerait la scalabilité.
\item Les données sont supposées tenir en mémoire après chunking ; pour des flux massifs en continu, adapter l'algorithme à un paradigme en flux serait nécessaire.
\item $K$ est fixe ; des algorithmes de sélection automatique de $K$ (silhouette, coude) pourraient être intégrés.
\end{enumerate}

\noindent\textbf{Perspectives :}
\begin{enumerate}
\item Porter l'implémentation sur Apache Spark pour bénéficier d'une distribution complète et d'une tolérance aux pannes.
\item Implémenter K-Means++ distribué (actuellement centralisé).
\item Ajouter d'autres métriques de qualité (Davies-Bouldin, Calinski-Harabasz) et d'autres initialisations (k-means||).
\item Évaluer la convergence avec des critères d'arrêt adaptatifs.
\end{enumerate}
...
par sommes partielles est une transformation mathématiquement exacte. La parallélisation
ne sacrifie donc aucune précision.

\item \textbf{Seuil de rentabilité.} MapReduce est avantageux en calcul pur dès
\textbf{50\,000 capteurs} ($2.41\times$, benchmark sur \texttt{large\_sensors.csv})
et dominant en scénario IoT réel dès 10\,000 capteurs (réseau 1\,Mbps). À 100\,000
capteurs, le gain total réseau inclus atteint déjà $12.2\times$.

\item \textbf{Efficacité réseau du Combiner.} Le Combiner est un facteur clé. En
réduisant le volume de $N$ enregistrements à $K$ sommes partielles, il divise le
trafic d'un facteur $N/K$ : de $\times 100$ à l'échelle testée jusqu'à $\times 3\,333$
à 50\,000 capteurs par serveur.

\item \textbf{Seuil critique à 500\,000 capteurs.} MapReduce est la seule option
viable pour les gros volumes. Avec le timeout de 20\,s de \texttt{benchmark.py},
l'algorithme classique échoue à partir de 500\,000 capteurs, tandis que MapReduce
termine dans tous les cas testés jusqu'à 1\,000\,000 capteurs (31.360\,s).

\item \textbf{Projection à l'échelle milliard.} À 1 milliard de capteurs, le classique
nécessite $\approx$3,4 jours (transfert + calcul) contre $\approx$8,7 heures pour
MapReduce ($9\times$ de gain). MapReduce \emph{termine} pendant que le classique
\emph{collecte encore les données}.

\end{enumerate}

En définitive, MapReduce pour K-Means constitue une solution viable et scalable pour
